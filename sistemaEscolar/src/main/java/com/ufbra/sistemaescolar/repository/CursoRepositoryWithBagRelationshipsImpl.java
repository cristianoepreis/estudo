...
public class CursoRepositoryWithBagRelationshipsImpl implements CursoRepositoryWithBagRelationships {
    ...
    public Optional<Curso> fetchBagRelationships(Optional<Curso> curso) {
        return curso.map(this::fetchDisciplinas);
    }
    ...
    public List<Curso> fetchBagRelationships(List<Curso> cursos) {
        return Optional.of(cursos).map(this::fetchDisciplinas).orElse(Collections.emptyList());
    }

    Curso fetchDisciplinas(Curso result) {
        return entityManager
            .createQuery("select curso from Curso curso left join fetch curso.disciplinas where curso.id = :id", Curso.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Curso> fetchDisciplinas(List<Curso> cursos) {
        ...
        List<Curso> result = entityManager
            .createQuery("select curso from Curso curso left join fetch curso.disciplinas where curso in :cursos", Curso.class)
