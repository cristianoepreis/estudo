...
import { DisciplinaService, EntityArrayResponseType } from '../service/disciplina.service';
...
export class DisciplinaComponent implements OnInit {
  ...
  trackId = (item: IDisciplina): number => this.disciplinaService.getDisciplinaIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.disciplinas = this.refineData(dataFromBody);
  }

  protected refineData(data: IDisciplina[]): IDisciplina[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IDisciplina[] | null): IDisciplina[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.disciplinaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
